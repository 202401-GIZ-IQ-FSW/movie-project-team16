...
import RecommendationsSlider from "@/app/components/slider/recomendation-slider";
...
function SingleMovie({ params }) {
  ...
  const getMovie = () => {
    ...
    GlobalApi.getMovieCredits(params.id).then((resp) => {
      ...
      setDirector(
        resp.data.crew.filter(({ job }) => job === "Director")[0].name
      );
    });
    ...
    GlobalApi.getMovieTrailer(params.id).then((resp) => {
      resp.data.results[0]
        ? setTrailerKey(resp.data.results[0].key)
        : setTrailerKey(false);
    });
  };
  return (
                ...
                {movie.production_companies?.map((production) => (
                  ...
                  >
                    {production.logo_path ? (
                      ...
                      <CgSmileNone className="text-white text-lg" />
                    )}
